...
public class TagView extends RelativeLayout {
    ...
    public TagView(Context context) {
        super(context, null);
        TagLogUtil.v(TAG,"[TagView]constructor 1");
        init(context, null, 0, 0);
    }

    public TagView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TagLogUtil.v(TAG,"[TagView]constructor 2");
        init(context, attrs, 0, 0);
    }

    public TagView(Context ctx, AttributeSet attrs, int defStyle) {
        super(ctx, attrs, defStyle);
        TagLogUtil.v(TAG,"[TagView]constructor 3");
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        TagLogUtil.v(TAG,"[onSizeChanged]w = " + w);
        mWidth = w;
        // drawTags();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        TagLogUtil.v(TAG,"[onMeasure]getMeasuredWidth = " + getMeasuredWidth());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        TagLogUtil.v(TAG,"[onDraw]");
        // drawTags();
    }

    @Override
    protected void onVisibilityChanged(@NonNull View changedView, int visibility) {
        TagLogUtil.v(TAG,"[onVisibilityChanged]");
        /*if (changedView == this){
    ...
    protected void onAttachedToWindow() {
        ...
        TagLogUtil.v(TAG,"[onAttachedToWindow]");
    }

    private void drawTags() {
        TagLogUtil.v(TAG,"[drawTags]visibility = " + (getVisibility() == View.VISIBLE));
        if (getVisibility() != View.VISIBLE) return;
        TagLogUtil.v(TAG,"[drawTags]mWidth = " + mWidth);
        TagLogUtil.d(TAG,"[drawTags]add tags, tag count = " + mTags.size());
        // clear all tag
        removeAllViews();
    ...
    public void addTag(Tag tag) {
        TagLogUtil.v(TAG,"[addTag]");
        mTags.add(tag);
        drawTags();
    }

    public void addTags(String[] tags) {
        TagLogUtil.v(TAG,"[addTags]");
    ...
    public void remove(int position) {
        TagLogUtil.v(TAG,"[remove]position = " + position);
        mTags.remove(position);
        drawTags();
    }

    public void removeAllTags() {
        TagLogUtil.v(TAG,"[removeAllTags]");
        mTags.clear();
        drawTags();
    }
